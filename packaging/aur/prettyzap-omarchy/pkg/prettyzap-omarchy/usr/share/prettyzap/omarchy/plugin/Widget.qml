...
Panel {
  ...
  readonly property string statusText: {
    ...
    if (data.running) return "Running" + (data.theme !== "" ? " · " + themeLabel() : "")
    return "Not running"
  }
  ...
  IpcHandler {
    ...
    function status(): string { return JSON.stringify({ running: data.running, theme: data.theme }) }
  }
  ...
  BarIconButton {
    ...
    tooltipText: data.installed
      ? "PrettyZap — " + (data.running ? "click to hide" : "click to open")
